...
function startMiningProcess(interval = 43200000) {
  setInterval(() => {
...
app.post('/send-otp', async (req, res) => {
  ...
  try {
    ...
    // Store the OTP in memory (should be replaced with a more secure method for production)
...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.json({ voterId, message: 'Voter registered successfully' });
  } catch (error) {
